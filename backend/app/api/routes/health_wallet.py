...
def _serialize(r: LabReport) -> dict:
    return {
        "id": str(r.id),
...
async def list_lab_reports(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    reports = (
        db.query(LabReport)
    ...
    grouped: dict = {}
    for r in reports:
        grouped.setdefault(r.test_name, []).append(_serialize(r))
    return grouped
...
async def get_trends(
    ...
    if len(reports) < 2:
        return {"message": "Need at least 2 data points", "test_name": test_name}
